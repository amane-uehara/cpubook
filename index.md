...
作る労力については、**CPUのコード行数がわずか69行** なので簡単に作れます。
要するに超単純で超低機能な CPU を作るのです。
強い CPU を作りたい方は、本書を読んだ後で更にハイレベルな本に挑戦すると良いでしょう。
...
この web サポートページでは Arty S7-50 を対象に説明します。

# いろいろな方への紹介文

本書の主な想定読者は、電気や回路や CPU について何も知らない方です。
しかし既に回路知識をお持ちの方々からも、 「こんな見方があるのか！」という驚きの声が多数届いています。
筆者として、本当に嬉しい限りです。
既存の論理回路の教科書とは、やや視点が異なる本なので、回路に詳しい方にも新たな発見があるのかもしれません。
以下では、様々なバックグラウンドの方に向けて10通りの紹介文をひねり出したので、興味のある項目に目を通してもらえると幸いです。
...
「実際に動かさないと意味がない」という硬派な意見もありますが、筆者の本音としては FPGA を買わずに本書を読むだけでも十分だと思っています。
本書のメインは FPGA で遊ぶことではなく、むしろ「CPUの作りかたを理解する」ことなのです。

まずは本書を手にとって、12章まで読んでみて下さい。
パソコン・ゲーム機・炊飯器・キッチンタイマーのようなデジタル家電の内部で、電子がどのように動いているのか直感的に理解できるでしょう。
これらのハードウェアが、どのように設計され、どのような言語で記述されるのか、だいたい理解できるはずです。
身の回りの電化製品を見渡すと、原理不明のブラックボックスだらけだと思いますが、これを「頑張れば自分でも設計できそう」まで引きずり下ろすのが、本書の目的なのです。
...
回路に対し、ある種の安定性を要求することで、自然にミーリー型回路やムーア型回路を導出します。
...
という回路図を用意すべきです。
紙に回路図を描くのです。
そして (4.2節の手順で) 回路図をコードに変換すれば、自動的に正しい HDL が生成されます。
...
のように話が進んでいきます。
これは「人間の欲望」に沿った展開といえます。
欲望を叶えるための回路を、各論的に提供するスタイルです。

一方、本書は「回路そのものの性質」に注目して議論を進めます。
そのため以下のように話が進みます。

* 2色で着色できる回路は? → 並列抵抗回路
* その中で、向き付け可能な回路は? → 論理回路
* その中で、向きが反転しない回路は? → 論理ゲートの回路
* その中で、安定性を持つ回路は? → ミーリー型回路
* その中で、再帰性を持つ回路は? → ムーア型回路
* こうして得られたムーア型回路は、CPUと呼ばれています

このように回路に対して段階的に制限を課すうちに、勝手にCPUが発生します。
そこに人間の欲望は存在しません。
しかし最終的に作る CPU は、人間の欲望の塊のような装置です。
この欲望は究極的に、CPU の命令セット (ISA) に集約されます。
こうした人間臭い話題を後半の章に隔離することで、ストーリー全体の見通しを良くしています。

↑などと書いても意味不明だと思いますので、本書のストーリーをむりやり算数に例えると
