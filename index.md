...
また色覚異常の方に配慮したバージョンのPDFファイルも提供しております。
書店購入時のレシートや領収書を出版社にメールしていただいた方に、ファイルを提供しております。
詳細は下記リンクをご参照ください。
...
* FPGA (USB で動く回路生成装置) を買えば、本物の CPU が作れる。
...
* 作業開始から**約2時間で CPU が完成**し、機械語が動く (開発環境の設定時間)。

コードの分かりやすさを重視した結果、完成する CPU が他書と比べてやや低機能になりました。
まず本書でコツをつかんでから、他書を参考に高機能な CPU を作るのが良いと思います。
「さすがに2時間では完成しないだろう」と思った方は、ぜひ
[回路を動かす手順](howto/index.md)
の作業を進めて下さい。
本当に2時間でCPUが動いてしまいます。

以下では本書で作るCPUを具体的に眺めてみます。
...
既存の論理回路の教科書とはストーリーがだいぶ違うので、回路に詳しい方にも新たな発見があるかもしれません。
...
Linux でも構いません。
しかし Mac で動かすのはかなり難しい(初心者には厳しい)です。
...
実際のところ、パソコン上に FPGA の開発環境を構築するのは、面倒な作業です。
また本書のメインは FPGA で遊ぶことではなく、むしろ「CPUの作りかたを理解する」ことです。
Mac をお持ちの方も、本書は読み物として十分楽しめると思います。
...
という回路図を用意すべきです。
紙に回路図を描くのです。
そして (本書の4.2節で導入する) アルゴリズムに沿って回路図を分解し、その各パーツをソースコードに置換すれば、自動的に正しい HDL が生成されます。
...
品薄状態が続いており、転売業者が高値で出品していますのでご注意ください。
